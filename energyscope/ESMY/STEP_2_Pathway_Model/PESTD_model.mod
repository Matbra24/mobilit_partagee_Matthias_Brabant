...
# MATTHIAS
param share_mobility_shared_min {YEARS} >= 0, <= 1 default 0; # %_shared,min [-]: min limit for penetration of public mobility over total mobility 
param share_mobility_shared_max {YEARS} >= 0, <= 1 default 0; # %_shared,max [-]: max limit for penetration of public mobility over total mobility 
...
var Share_mobility_shared {y in YEARS} >= share_mobility_shared_min [y], <= share_mobility_shared_max [y]; # %_Shared: Ratio [0; 1] shared mobility over total passenger mobility
...
subject to end_uses_t {y in YEARS_WND diff YEAR_ONE, l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	End_uses [y,l, h, td] = (if l == "ELECTRICITY" 
		then
			...
			(end_uses_input[y,"MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * (Share_mobility_public [y] + 0.0062 * Share_mobility_shared [y])
		else (if l == "MOB_SHARED" then
			(end_uses_input[y,"MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * Share_mobility_shared [y] * (1 - 0.0062)
		else (if l == "MOB_PRIVATE" then
			(end_uses_input[y,"MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * (1 - Share_mobility_public [y] - Share_mobility_shared [y] )
...
subject to storage_layer_out {y in YEARS_WND diff YEAR_ONE, j in STORAGE_TECH, l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	(if storage_eff_out [y, j, l]=0 then  Storage_out [y, j, l, h, td]  = 0);
		
# [Eq. 19] limit the Energy to power ratio. 
subject to limit_energy_to_power_ratio {y in YEARS_WND diff YEAR_ONE, j in STORAGE_TECH diff {"BEV_BATT","PHEV_BATT","BEV_BATT_SHARED","PHEV_BATT_SHARED"}, l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
...
# [Eq. XX] Intialise the first phase based on YEAR_2025 results
subject to F_new_initialisation {tech in TECHNOLOGIES}:
	F_new ["2020_2025",tech] = F["YEAR_2025",tech]; # Generate F_new2020_2025
