...
class AmplObject:
    ...
    def get_elem(self, elem_name: str, type_of_elem='Var'):
        ...
        amplpy_df = ampl_elem.getValues()
        n_indices = amplpy_df._getNumIndices()  # ← changement ici
        ...
        if type_of_elem == 'Var':
            elem.rename(columns=lambda x: x[:-4] if x.endswith('.val') else x, inplace=True)
            self.outputs[elem_name] = elem
        ...
        try:
            ...
            for m in mod_2_path:
                if not os.path.exists(m):
                    open(m,"w+")
                ampl.read(m)
                        # === DEBUG: Vérification des techno et couches ===
            print("\n=== Technologies ===")
            print(ampl.get_set("TECHNOLOGIES").to_list())
    ...
    def to_pd(amplpy_df):
        ...
        headers = amplpy_df._getHeaders()  # Nouveau nom de méthode
        columns = {header: list(amplpy_df._getColumn(header)) for header in headers}  # idem ici

        df = pd.DataFrame(columns)
        df = df.rename(columns={headers[-1]: 'Value'})  # la dernière colonne contient les valeurs numériques
        df = df.set_index(list(headers[:-1]))  # on met les indices en index
        df.index.name = None  # supprime le nom de l'index multi-index
    ...
    def get_assets(self):
        ...
        sto_tech_daily.remove('BEV_BATT_SHARED')
        sto_tech_daily.remove('PHEV_BATT_SHARED')
