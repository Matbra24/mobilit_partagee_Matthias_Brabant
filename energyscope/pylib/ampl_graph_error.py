...
class AmplGraph:
    ...
    def graph_layer(self, ampl_collector = None, plot = True):
        ...
        col_plot = ['AMMONIA','ELECTRICITY','GAS','H2','WOOD','WET_BIOMASS',
                    ...
                    'MOB_FREIGHT_ROAD','MOB_PRIVATE','MOB_SHARED','MOB_PUBLIC']
        ...
        for k in results.columns:
            ...
            for y in results.index.get_level_values(0).unique():
                ...
                if not temp_y.empty:
                    ...
                    if not temp_y.empty:
                        ...
                        temp_pos = temp_pos.groupby(['Years'])[k].sum().values[0]
                        temp_neg = temp_df[(temp_df[k]<0) & (temp_df['Years']==y)]
                        temp_neg = temp_neg.groupby(['Years'])[k].sum().values[0]
            ...
            if plot:
                ...
                if k in ['GAS','H2','WOOD','WET_BIOMASS']:
                    ...
                    print(self.ampl_collector['Year_balance'].head())



                    cond_2025 = (df_to_plot['Elements'] == 'END_USES') & (df_to_plot['Years'] == '2025')
                    cond_2050 = (df_to_plot['Elements'] == 'END_USES') & (df_to_plot['Years'] == '2050')

                    EUD_2025 = df_to_plot.loc[cond_2025, k].values[0] if not df_to_plot.loc[cond_2025].empty else 0
                    EUD_2050 = df_to_plot.loc[cond_2050, k].values[0] if not df_to_plot.loc[cond_2050].empty else 0
                ...
                vals = []
                if not temp_neg[k].empty:
                    vals.append(round(temp_neg[k].min(), 1))
                if EUD_2025:
                    vals.append(round(EUD_2025, 1))
                if EUD_2050:
                    vals.append(round(EUD_2050, 1))
                vals.append(0)
                if not temp_pos[k].empty:
                    vals.append(round(temp_pos[k].max(), 1))

                if vals:  # éviter sorted([]) = []
                    yvals = sorted(vals)
                else:
                    yvals = [0]
    ...
    def graph_gwp_per_sector(self, ampl_collector = None, plot = True):
        layers = ['GAS','H2','AMMONIA','METHANOL','ELECTRICITY','HEAT_HIGH_T',
                ...
                'MOB_PRIVATE','MOB_SHARED','MOB_PUBLIC']
        ...
        for eud in update_gwp_remaining:
            ...
            temp['gwp_op'] = 0
            # DEBUG - identifier les doublons dans l'index
            print("Doublons dans gwp_op ?", gwp_op.index.duplicated().any())
            print("Doublons dans temp[...].index ?", temp.loc[temp.index.get_level_values('Elements').isin(resources)].index.duplicated().any())
    ...
    def graph_cost(self):
        ...
        print('tyeps',df_to_plot.dtypes)
        print('colums', df_to_plot.columns)
        df_to_plot['Cost'] = pd.to_numeric(df_to_plot['Cost'], errors='coerce')


        
        title = "<b>Annual system cost</b><br>[b€<sub>2015</sub>/y]"
        temp = df_to_plot.groupby(['Years']).sum(numeric_only=True)
        print('head',temp.head())
    ...
    def graph_cost_return(self, ampl_collector = None, plot = True):
        ...
        df_to_plot_eff[df_to_plot_eff.columns] = df_to_plot_eff[df_to_plot_eff.columns].apply(pd.to_numeric, errors='coerce')
        ...
        if plot:
            ...
            numeric_cols = df_to_plot_eff.select_dtypes(include='number').columns

            # Groupby et somme uniquement sur celles-ci
            temp = df_to_plot_eff.groupby('Years')[numeric_cols].sum()

            # Puis extraire la colonne 'Cost_return'
            temp = temp['Cost_return']
    ...
    def graph_cost_op_phase(self, ampl_collector = None, plot=True):
        ...
        if plot:
            ...
            # MATTHIAS Sélectionne uniquement les colonnes numériques
            numeric_cols = df_to_plot.select_dtypes(include='number').columns

            # Agrège uniquement celles-ci
            temp = df_to_plot.groupby('Years')[numeric_cols].sum().reset_index()
    ...
    def graph_load_factor_scaled(self, ampl_collector = None, plot = True):
        ...
        for tech in c_p_inter.index.get_level_values('Technologies').unique():
            try:
                # Get scalar value from c_p_inter safely
                value = c_p_inter.xs(tech, level='Technologies').values[0].item()
                
                # Assign scalar value across all years for the given tech
                c_p.loc[(slice(None), tech), 'c_p'] = value
            except KeyError as e:
                print(f"Skipping tech {tech}: {e}")
            except ValueError as e:
                print(f"ValueError for tech {tech} (maybe multiple values?): {e}")
    ...
    def _compute_transition_cost(self, ampl_collector):
        # 1. Get Tot_capex and ensure numeric types
        Tot_capex = self.graph_cost_inv_phase_tech(ampl_collector, plot=False)
        Tot_capex = Tot_capex.set_index('Years')
        Tot_capex['C_inv_phase_tech'] = pd.to_numeric(Tot_capex['C_inv_phase_tech'], errors='coerce')

        # Optional: compute cumsum before grouping (if needed downstream)
        Tot_capex['cumsum'] = Tot_capex.groupby(Tot_capex.index)['C_inv_phase_tech'].cumsum()

        # Drop categorical columns before sum
        Tot_capex = Tot_capex.select_dtypes(include='number').groupby(Tot_capex.index).sum()

        # 2. Tot_opex: do the same
        Tot_opex = self.graph_cost_op_phase(ampl_collector, plot=False)
        Tot_opex = Tot_opex.set_index('Years')
        Tot_opex = Tot_opex.select_dtypes(include='number').groupby(Tot_opex.index).sum()

        # 3. Tot_return
        Tot_return = self.graph_cost_return(ampl_collector, plot=False)
        Tot_return = Tot_return.set_index('Years')
        Tot_return = Tot_return.select_dtypes(include='number').groupby(Tot_return.index).sum()

        print("Tot_capex columns:", Tot_capex.columns)
        print("Tot_opex columns:", Tot_opex.columns)
        print("Tot_capex head:\n", Tot_capex.head())
        print("Tot_opex head:\n", Tot_opex.head())

        # 4. Final Transition Cost calculation
        Trans_cost = Tot_capex['cumsum'] + Tot_opex['cumsum'].sub(Tot_return['Cost_return'], fill_value=0)
        ...
        for k in categories:
            for j in categories[k]:
                if k == 'HEAT_LOW_T':
                    ...
                    if j in self.ampl_obj.sets['TECHNOLOGIES_OF_END_USES_TYPE']["MOB_SHARED"]:
    ...
    def _dict_layer_unit(self):
        layers = ['AMMONIA','ELECTRICITY','GAS','H2','WOOD','WET_BIOMASS','HEAT_HIGH_T',
                    ...
                    'MOB_SHARED','MOB_PUBLIC']

        dict_layer_unit =  dict.fromkeys(layers)
        for sector in layers:
            if sector in ['MOB_PRIVATE','MOB_SHARED','MOB_PUBLIC']:
    ...
    def dict_color(category):
        ...
        if category == 'Electricity':
            ...
            color_dict = {"ELECTRICITY" : "deepskyblue", "HEAT_HIGH_T":"red","HEAT_LOW_T_DECEN":"lightpink", "HEAT_LOW_T_DHN":"indianred", "MOB_PUBLIC":"gold", "MOB_PRIVATE":"goldenrod", "MOB_SHARED":"red","MOBILITY_FREIGHT":"darkgoldenrod", "NON_ENERGY": "darkviolet", "INFRASTRUCTURE":"grey","HVC":"cyan",'STORAGE':'chartreuse', 'OTHERS':'gainsboro'}
    ...
    def custom_fig(fig,title,yvals,xvals=['2025','2030','2035','2040',
        ...
        if len(fig.data) > 0 and len(fig.data[0].x) > 0:
            ...
            print("⚠️ Graphe vide : aucune donnée trouvée pour cette couche.")
            return
        ...
        print("tickvals avant tri :", fig.layout.xaxis.tickvals)
        ...
        try:
            ...
            # Vérifie si tous sont numériques
            if all(isinstance(v, (int, float)) for v in tickvals):
                fig.layout.xaxis.tickvals = sorted([float(v) for v in tickvals])
            elif all(isinstance(v, str) for v in tickvals):
                fig.layout.xaxis.tickvals = sorted(tickvals)
            else:
                # Mélange types => conversion en str
                fig.layout.xaxis.tickvals = sorted([str(v) for v in tickvals])
