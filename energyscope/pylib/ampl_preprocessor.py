...
class AmplPreProcessor:
    ...
    def remaining_update(self, i, file_in='PES_data_remaining.dat', file_out='PES_data_remaining_wnd.dat'):
        ...
        with open(pth_file, encoding='utf-8') as fp:
            ...
            phase_columns = header[1:]  # skip the first column (technology)

            phase_indices = [phase_columns.index(p) for p in phase_list if p in phase_columns]

            if len(phase_indices) != len(phase_list):
                missing = set(phase_list) - set(phase_columns)
                raise ValueError(f"Missing phase(s) in input data: {missing}")
        ...
        with open(os.path.join(pth_model, file_out), 'w+', encoding='utf-8') as f:
            ...
            for line in lines:
                ...
                print(f"{parts[0]}: {parts}")  # Ajoute cette ligne

                if len(parts) < 2:
                    continue
                tech = parts[0]
                try:
                    values = parts[1:1 + len(phase_indices)]
                except IndexError:
                    raise ValueError(f"Ligne incomplète pour la technologie '{tech}' dans le fichier {file_in}.")
