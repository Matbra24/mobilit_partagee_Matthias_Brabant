...
class AmplGraph:
    ...
    def graph_layer(self, ampl_collector = None, plot = True):
        ...
        col_plot = ['AMMONIA','ELECTRICITY','GAS','H2','WOOD','WET_BIOMASS',
                    ...
                    'MOB_FREIGHT_ROAD','MOB_PRIVATE','MOB_PUBLIC']#,'MOB_SHARED'
        ...
        for k in results.columns:
            ...
            for y in results.index.get_level_values(0).unique():
                ...
                if not temp_y.empty:
                    ...
                    if not temp_y.empty:
                        ...
                        # Convertir toutes les colonnes sauf 'Years' en float
                        cols_to_convert = temp_pos.columns.difference(['Years']) if 'Years' in temp_pos.columns else temp_pos.columns
                        temp_pos[cols_to_convert] = temp_pos[cols_to_convert].apply(pd.to_numeric, errors='coerce')

                        # Optionnel : remplir les NaN avec 0
                        temp_pos.fillna(0, inplace=True)

                        temp_pos = temp_pos.groupby(['Years']).sum()[k].values[0]
                        temp_neg = temp_df[(temp_df[k]<0) & (temp_df['Years']==y)]
                        # Convertir toutes les colonnes sauf 'Years' en numérique
                        cols_to_convert = temp_neg.columns.difference(['Years']) if 'Years' in temp_neg.columns else temp_neg.columns
                        temp_neg[cols_to_convert] = temp_neg[cols_to_convert].apply(pd.to_numeric, errors='coerce')

                        # Optionnel : Remplacer les NaN par 0
                        temp_neg.fillna(0, inplace=True)
            ...
            if plot:
                ...
                if k in ['GAS','H2','WOOD','WET_BIOMASS']:
                    ...
                    print(df_to_plot['Years'].unique(), df_to_plot['Years'].dtype)

                    print(df_to_plot['Elements'].unique())
                    df_to_plot['Elements'] = df_to_plot['Elements'].str.strip()
    ...
    def graph_gwp_per_sector(self, ampl_collector = None, plot = True):
        layers = ['GAS','H2','AMMONIA','METHANOL','ELECTRICITY','HEAT_HIGH_T',
                ...
                'MOB_PRIVATE','MOB_PUBLIC','MOB_SHARED']
        mob_freight_layers = ['MOB_FREIGHT_BOAT','MOB_FREIGHT_ROAD',
                              'MOB_FREIGHT_RAIL']
        
        CO2_layers = ['CO2_ATM','CO2_CAPTURED','CO2_INDUSTRY']
        
        order_entry = ['MOB_PRIVATE','MOB_PUBLIC','MOB_SHARED','MOBILITY_FREIGHT',
                       'ELECTRICITY','HEAT_HIGH_T','HEAT_LOW_T_DHN',
                       'HEAT_LOW_T_DECEN','HVC','METHANOL','AMMONIA']#,'MOB_SHARED'
        ...
        for eud in update_gwp_remaining:
            ...
            gwp_op = gwp_op._append(gwp_op_3)
        ...
        gwp_per_layer = others._append(mob_freight)
        ...
        gwp_per_layer = gwp_per_layer._append(year_balance_sto)
        gwp_per_layer.reset_index(inplace=True)
        gwp_per_layer['Years'] = gwp_per_layer['Years'].astype(str).str.replace('YEAR_', '')
        ...
        if plot:
            print("Catégories dans 'Layers':", gwp_per_layer['Layers'].cat.categories if gwp_per_layer['Layers'].dtype.name == 'category' else gwp_per_layer['Layers'].unique())
            print("Valeurs présentes :", gwp_per_layer['Layers'].unique())

            # Supprimer les catégories inutilisées dans la colonne 'Layers'
            if gwp_per_layer['Layers'].dtype.name == 'category':
                gwp_per_layer['Layers'] = gwp_per_layer['Layers'].cat.remove_unused_categories()
    ...
    def graph_cost(self):
        ...
        order_entry = ['INFRASTRUCTURE','STORAGE','MOB_PRIVATE','MOB_PUBLIC','MOB_SHARED',
                       'MOBILITY_FREIGHT','ELECTRICITY','HEAT_HIGH_T',
                       'HEAT_LOW_T_DHN','HEAT_LOW_T_DECEN','HVC','METHANOL',
                       'AMMONIA','RE_FUELS','NRE_FUELS']#,'MOB_SHARED'
        ...
        """    
        # S'assurer que 'Years' n'est pas un type catégoriel
        df_to_plot['Years'] = df_to_plot['Years'].astype(str).astype(int)

        # Sélectionner uniquement les colonnes numériques pour la somme
        numeric_cols = df_to_plot.select_dtypes(include='number').columns
        temp = df_to_plot.groupby('Years')[numeric_cols].sum()
        print(temp.columns)
        print(temp.head())
        ...
        plt.close()
        """
        # Convertir la colonne 'Years' en type entier si ce n'est pas déjà fait
        df_to_plot['Years'] = df_to_plot['Years'].astype(str).astype(int)

        # Vérification si la colonne 'Cost' est présente dans df_to_plot
        if 'Cost' in df_to_plot.columns:
            print("La colonne 'Cost' existe dans df_to_plot.")
        else:
            print("La colonne 'Cost' est absente dans df_to_plot.")

        # Sélectionner uniquement les colonnes numériques, en particulier 'Cost'
        numeric_cols = df_to_plot.select_dtypes(include=['number']).columns

        # Agréger en utilisant 'Years' et la colonne 'Cost' (ainsi que les autres colonnes numériques)
        temp = df_to_plot.groupby(['Years', 'Cost'])[numeric_cols].sum().reset_index(drop=True)


        # Vérifier les colonnes après l'agrégation
        print("Colonnes après l'agrégation :", temp.columns)





        # Sélectionner uniquement les colonnes numériques pour effectuer la somme
        numeric_cols = df_to_plot.select_dtypes(include=['number']).columns
        print("test chat",df_to_plot['Years'].unique())


        # Faire la somme uniquement sur les colonnes numériques
        temp = df_to_plot.groupby('Years')[['Cost']].sum().reset_index()

        print("temps", temp.columns)

        # Afficher les colonnes après la somme pour vérifier
        print("Colonnes après l'agrégation :", temp.columns)
        print(temp.head())
        temp.columns = temp.columns.str.strip()  # Retirer les espaces autour des noms de colonnes

        temp.set_index('Years', inplace=True)



        # Définir les valeurs de l'axe des ordonnées pour le graphique
        yvals = sorted([0, min(round(temp['Cost'], 1)),
                        round(temp.loc[2025, 'Cost'], 1),  # Accéder à la valeur pour l'année 2025
                        max(round(temp['Cost'], 1))])
        '''
        if 'Cost' in temp.columns:
            # Définir les valeurs de l'axe des ordonnées pour le graphique
            yvals = sorted([0, min(round(temp['Cost'], 1)),
                            round(temp.loc[2025, 'Cost'], 1),  # Accéder à la valeur pour l'année 2025
                            max(round(temp['Cost'], 1))])
        else:
            print("La colonne 'Cost' n'existe pas dans le DataFrame.")
            # Gestion du cas où 'Cost' n'est pas présente, par exemple en affichant un message d'erreur ou en choisissant une autre colonne
            yvals = sorted([0, min(temp.min(), 1), temp.max()])
        '''
        # Appliquer la fonction personnalisée pour ajouter un titre et définir les valeurs de l'axe y
        self.custom_fig(fig, title, yvals)

        # Sauvegarder le graphique en HTML et en PDF
        fig.write_html(self.outdir + "_Raw/System_cost.html")
        fig.write_image(self.outdir + "System_cost.pdf", width=1200, height=550)

        # Fermer le plot matplotlib
        plt.close()
    ...
    def graph_cost_return(self, ampl_collector = None, plot = True):
        ...
        # Convertir toutes les colonnes (sauf 'Years' si présente) en numérique
        cols_to_convert = df_to_plot_eff.columns.difference(['Years']) if 'Years' in df_to_plot_eff.columns else df_to_plot_eff.columns
        df_to_plot_eff[cols_to_convert] = df_to_plot_eff[cols_to_convert].apply(pd.to_numeric, errors='coerce')
        ...
        if plot:
            ...
            # Identifier les colonnes numériques (et 'Years' pour le groupby)
            cols_to_convert = df_to_plot_eff.columns.difference(['Years']) if 'Years' in df_to_plot_eff.columns else df_to_plot_eff.columns

            # Convertir en float (ignorer les erreurs, remplacer les non convertibles par NaN)
            df_to_plot_eff[cols_to_convert] = df_to_plot_eff[cols_to_convert].apply(pd.to_numeric, errors='coerce')

            # (Optionnel) remplacer les NaN par 0 si nécessaire
            df_to_plot_eff.fillna(0, inplace=True)

            # Puis faire le groupby + somme
    ...
    def graph_cost_inv_phase_tech(self, ampl_collector = None, plot=True):
        ...
        if plot:
            ...
            # Garder uniquement les colonnes numériques (int ou float)
            numeric_cols = df_to_plot_full.select_dtypes(include=['number']).columns

            # Inclure Years si elle n'est pas déjà dans numeric_cols
            group_cols = ['Years'] + [col for col in numeric_cols if col != 'Years']

            # Grouper et sommer uniquement les colonnes numériques
            temp = df_to_plot_full[group_cols].groupby('Years').sum().reset_index()

            #
            # 
            # temp = df_to_plot_full.groupby(['Years']).sum()

            temp['cumsum'] = temp[numeric_cols].sum(axis=1).cumsum()
            temp['cumsum'] = pd.to_numeric(temp['cumsum'], errors='coerce')  # for safety
    ...
    def graph_cost_op_phase(self, ampl_collector = None, plot=True):
        ...
        order_entry = ['NRE_FUELS','RE_FUELS',
                       'INFRASTRUCTURE','STORAGE','MOB_PRIVATE','MOB_PUBLIC','MOB_SHARED',
                       'MOBILITY_FREIGHT','ELECTRICITY','HEAT_HIGH_T',
                       'HEAT_LOW_T_DHN','HEAT_LOW_T_DECEN','HVC','METHANOL',
                       'AMMONIA']#,'MOB_SHARED'
        ...
        if plot:
            ...
            # Grouper et sommer uniquement les colonnes numériques
            temp = df_to_plot[group_cols].groupby('Years').sum().reset_index()
    ...
    def graph_total_cost_per_year(self, ampl_collector = None, plot = True):        
        ...
        df_to_plot['index'] = df_to_plot['index'].astype(str).str.replace('YEAR_', '')
    ...
    def graph_tech_cap(self, ampl_collector = None, plot = True):
        ...
        for sector, tech in dict_tech.items():
            ...
            if len(df_to_plot_full) == 0:
                ...
                df_to_plot_full = df_to_plot_full._append(df_to_plot)
    ...
    def graph_load_factor_scaled(self, ampl_collector = None, plot = True):
        ...
        for tech in c_p_inter.index:
            print("Index types c_p      :", type(c_p.index))
            print("Index types c_p_inter:", type(c_p_inter.index))
            print("Sample tech in c_p   :", ('YEAR_2015', 'DEC_SOLAR') in c_p.index)
            print("Sample tech in c_p_inter:", ('YEAR_2015', 'DEC_SOLAR') in c_p_inter.index)

            print("Index names c_p      :", c_p.index.names)
            print("Index names c_p_inter:", c_p_inter.index.names)

            if tech in c_p_inter.index:
                value = c_p_inter.loc[tech].values[0]
                if tech in c_p.index:
                    c_p.loc[tech, 'c_p'] = value
                else:
                    print(f"⚠️ {tech} absent de l’index de c_p")
            else:
                print(f"⚠️ {tech} absent de l’index de c_p_inter")
    ...
    def graph_comparison(self,output_files,type_of_graph):
        ...
        if type_of_graph not in ['Layer','Load_factor']:
            # Filtrer uniquement les colonnes numériques avant de faire la soustraction
            numeric_cols = result_0.select_dtypes(include='number').columns

            # Effectuer la soustraction uniquement sur ces colonnes
            df_numeric_diff = result_1[numeric_cols].sub(result_0[numeric_cols], fill_value=0)

            # Si tu veux conserver les colonnes non numériques (comme 'Elements' pour le graph), les rajouter :
            if 'Elements' in result_1.columns:
                df_to_plot = result_1[['Elements']].copy()
                df_to_plot = pd.concat([df_to_plot, df_numeric_diff], axis=1)
            else:
                df_to_plot = df_numeric_diff

            df_to_plot.reset_index(inplace=True)

        if type_of_graph == 'C_inv_phase_tech':
            df0 = self.graph_cost_inv_phase_tech(self.unpkl(self, output_files[0]), plot=False)
            df1 = self.graph_cost_inv_phase_tech(self.unpkl(self, output_files[1]), plot=False)

            # S'assurer que les deux DataFrames ont les mêmes index
            df0 = df0.set_index(['Years', 'Category'])
            df1 = df1.set_index(['Years', 'Category'])

            # Conserver uniquement les colonnes numériques
            numeric_cols = df0.select_dtypes(include='number').columns

            # Calculer la différence
            df_diff = df1[numeric_cols].sub(df0[numeric_cols], fill_value=0).reset_index()

            # Pour le graphique, on suppose que 'cumsum' est bien dans les colonnes calculées
            df_to_plot = df_diff

        if type_of_graph in ['C_op_phase']:
            # Conversion des colonnes numériques si elles sont mal typées
            for col in ['cumsum', 'C_inv_phase_tech']:
                if col in df_to_plot.columns:
                    df_to_plot[col] = pd.to_numeric(df_to_plot[col], errors='coerce')
                    # Filtrer uniquement les coûts opérationnels de la mobilité privée et partagée
                    df_to_plot = df_to_plot[df_to_plot['Category'].isin(['MOB_PRIVATE', 'MOB_SHARED'])]


            # S'assurer que les colonnes nécessaires sont présentes et bien typées
            for col in ['Years', 'Category', 'cumsum']:
                if col not in df_to_plot.columns:
                    raise ValueError(f"La colonne {col} est manquante dans df_to_plot")

            # Convertir les colonnes numériques si nécessaire
            df_to_plot['cumsum'] = pd.to_numeric(df_to_plot['cumsum'], errors='coerce')

            # S'assurer que 'Years' et 'Category' sont bien des chaînes
            df_to_plot['Years'] = df_to_plot['Years'].astype(str)
            df_to_plot['Category'] = df_to_plot['Category'].astype(str)
            ...
            df0 = self.graph_gwp_per_sector(self.unpkl(self, output_files[0]), plot=False)
            df1 = self.graph_gwp_per_sector(self.unpkl(self, output_files[1]), plot=False)

            # Vérifier les colonnes
            print("DF0 columns:", df0.columns)
            print("DF1 columns:", df1.columns)

            # Assurer l’alignement sur les bonnes colonnes d’index
            df0 = df0.set_index(['Years', 'Layers'])
            df1 = df1.set_index(['Years', 'Layers'])

            # Sélectionner les colonnes numériques
            numeric_cols = ['GWP_EUD'] if 'GWP_EUD' in df0.columns else df0.select_dtypes(include='number').columns

            # Calcul de la différence
            df_diff = df1[numeric_cols].sub(df0[numeric_cols], fill_value=0).reset_index()
            # Filtrer uniquement les couches de mobilité et électricité
            desired_layers = ['MOB_PRIVATE', 'MOB_PUBLIC', 'MOB_SHARED', 'ELECTRICITY','HVC','HEAT_HIGH_T']
            df_diff = df_diff[df_diff['Layers'].isin(desired_layers)]
            df_diff['Layers'] = df_diff['Layers'].astype(str)  # Supprimer les catégories inutilisées'''
            ...
            # Assure que le résultat est bien un DataFrame avec colonnes 'Years' et 'Tot_trans_cost'
            result_0 = result_0.reset_index()
            result_0.columns = ['Years', 'Tot_trans_cost']

            result_1 = result_1.reset_index()
            result_1.columns = ['Years', 'Tot_trans_cost']

            df_to_plot = result_1.set_index('Years').sub(result_0.set_index('Years'), fill_value=0).reset_index()
    ...
    def _compute_transition_cost(self, ampl_collector):
        ...
        # Mise en forme : aligner sur 'Years'
        Tot_capex = Tot_capex.groupby('Years', as_index=True)['cumsum'].sum()
        Tot_opex = Tot_opex.groupby('Years', as_index=True)['cumsum'].sum()
        Tot_return = Tot_return.groupby('Years', as_index=True)['Cost_return'].sum()

        # Calcul final des coûts de transition
        Trans_cost = Tot_capex + Tot_opex.sub(Tot_return, fill_value=0)
        ...
        for k in categories:
            for j in categories[k]:
                if k == 'HEAT_LOW_T':
                    ...
                    elif j in self.ampl_obj.sets['TECHNOLOGIES_OF_END_USES_TYPE']["MOB_SHARED"]:
                        categories_2[j] = 'MOB_SHARED'
                    else:
    ...
    def _dict_layer_unit(self):
        layers = ['AMMONIA','ELECTRICITY','GAS','H2','WOOD','WET_BIOMASS','HEAT_HIGH_T',
                    ...
                    'MOB_PUBLIC','MOB_SHARED']

        dict_layer_unit =  dict.fromkeys(layers)
        for sector in layers:
            if sector in ['MOB_PRIVATE','MOB_PUBLIC','MOB_SHARED']:
    ...
    def dict_color(category):
        ...
        if category == 'Electricity':
            ...
            color_dict = {"ELECTRICITY" : "deepskyblue", "HEAT_HIGH_T":"red","HEAT_LOW_T_DECEN":"lightpink", "HEAT_LOW_T_DHN":"indianred", "MOB_PUBLIC":"gold", "MOB_PRIVATE":"goldenrod","MOB_SHARED": "green","MOBILITY_FREIGHT":"darkgoldenrod", "NON_ENERGY": "darkviolet", "INFRASTRUCTURE":"grey","HVC":"cyan",'STORAGE':'chartreuse', 'OTHERS':'gainsboro'}
